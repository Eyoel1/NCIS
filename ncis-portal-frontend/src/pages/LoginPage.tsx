...
export const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Find role to redirect correctly
        const matched = SEED_USERS_LIST.find(u => u.email.toLowerCase() === loginEmail.trim().toLowerCase());
        const targetPath = getRolePath(matched ? matched.role : 'IMPORTER_SUPPLIER');
        navigate(targetPath);
      } else {
        setLoginError('Invalid credentials. Please verify your email or use a seed account below.');
      }
    } catch {
        ...
        <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6 bg-slate-950/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-md">
          ...
          <div className="lg:col-span-5 bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 space-y-3 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                <div className="flex items-center gap-1.5 text-xs font-bold text-slate-200">
                  ...
                  <span>Seed Accounts Directory</span>
                </div>
                ...
                {SEED_USERS_LIST.map((seed) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-2 rounded-xl border border-slate-800 hover:border-slate-700 bg-slate-950/60 hover:bg-slate-900 cursor-pointer transition-all card-hover-lift group flex items-center justify-between"
                    >
                      ...
                      <button
                        type="button"
                        className="shrink-0 p-1.5 rounded-lg bg-slate-800 text-slate-400 group-hover:text-white transition"
            ...
            <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800/80 text-[10px] text-slate-400">
              💡 <strong>Tip:</strong> You can also create your own custom account using the <strong>Create Account</strong> tab!
            </div>
          </div>
